...
export async function updateUser(id:string,input:{fullName?:string;locale?:string}){return db.user.update({where:{id},data:{...input,...(input.fullName?{firstName:input.fullName.split(' ')[0],lastName:input.fullName.split(' ').slice(1).join(' ')}:{})}});}
